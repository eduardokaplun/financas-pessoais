...
const DEFAULT_CATS = [
  ...
  {nome:'Investimentos',emoji:'📈',tipo:'receita'},
...
export default function Dashboard({ user, onLogout }) {
  ...
  function ItemRow({item, showDel=false}) {
    ...
    const cls = item.tipo==='receita'?'g':isParc?'p':isFixo?'y':'r'
    return (
      <div className="item">
        <div className={`item-ico ${cls}`}>{catEmoji(item.categoria,cats)}</div>
        ...
        <div className="item-right">
          <div className={`item-val ${cls}`}>{item.tipo==='receita'?'+':'-'}{fmt(item.valor)}</div>
        </div>
        {showDel && !isFixo && <button className="del-btn" onClick={()=>delLancamento(item)}>×</button>}
      </div>
    )
  }
    ...
    <>
      ...
      <div className={`page${tab==='lancamentos'?' on':''}`}>
        ...
        <div className="filters">
          {[['todos','Todos'],['receita','Receitas'],['despesa','Despesas'],['parcelas','Parcelas'],['fixos','Fixos']].map(([f,label])=>(
          ...
          <div className="card">
            {trend.map((x,i)=>(
              <div className="trend-row" key={i}>
                ...
                <div className="trend-vals"><span style={{color:'var(--g)'}}>{fmtShort(x.rec)}</span> / <span style={{color:'var(--r)'}}>{fmtShort(x.desp)}</span></div>
              </div>
            ))}
          </div>
        </div>
        <div className="sec" style={{marginTop:16}}>
          <div className="sec-title">Top categorias (geral)</div>
          <div className="card">
          ...
          <div className="card">
            ...
            {fixos.map(f=>(
              <div className="fixo-item" key={f.id}>
                <div className={`item-ico ${f.tipo==='receita'?'g':'y'}`}>{catEmoji(f.categoria,cats)}</div>
                <div className="item-info">
            ...
            {cats.map(c=>(
              <div className="item" key={c.id||c.nome}>
                ...
                <div className="item-info">
                  ...
                  <div className="item-sub">{c.tipo==='receita'?'Entrada':'Saída'}</div>
                </div>
        ...
        <div className="overlay open" onClick={e=>e.target.className==='overlay open'&&setModal(null)}>
          <div className="modal">
            ...
            <div className="tipo-toggle">
              <button className={`tipo-btn${tipo==='receita'?' on-rec':''}`} onClick={()=>changeTipo('receita')}>💰 Receita</button>
              <button className={`tipo-btn${tipo==='despesa'?' on-des':''}`} onClick={()=>changeTipo('despesa')}>💸 Despesa</button>
            </div>
            <div className="form-card">
              <div className="ff"><label>O quê</label><input placeholder="ex: biscoito, salário..." value={desc} onChange={e=>setDesc(e.target.value)} /></div>
